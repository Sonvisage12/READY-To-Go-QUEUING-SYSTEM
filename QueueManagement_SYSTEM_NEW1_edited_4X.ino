...
void notifyDoctorsOfNewMaster() {
    ...
    QueueItem announce = {};
    String myMAC = WiFi.macAddress();
    myMAC.toCharArray(announce.sourceMAC, sizeof(announce.sourceMAC));  // include new master's MAC
    strcpy(announce.uid, "MASTER_CHANGE");       // special identifier for master change
    announce.number = doctorID;
    WiFi.macAddress(announce.sourceMAC);                   // new master's index
    announce.addToQueue = false;
    announce.removeFromQueue = false;
    strcpy(announce.queueID, "");                // not used for this message
    strcpy(announce.type, "MASTER");             // optional: indicate message type

    // Send to each doctor node (i+1 is the doctor node ID)
    for (int i = 0; i < numDoctorNodes; ++i) {
        announce.node = i + 1;  // target doctor node ID
        esp_now_send(doctorMAC, (uint8_t*)&announce, sizeof(announce));
    }
    Serial.printf("📢 New master announced (index %d, MAC %s) to all doctors.\n", myIndex, announce.sourceMAC);
}
